...
int index_directory(char *dir, char *title)
{
    ...
    printf("<html>\n"//为什么这里不能输出？？
	   ...
	   "<html>\n"//这里才可以 这是一种丢失现象？for what?
           "<head>\n<meta charset=\"utf-8\"><title>Index of DownPage</title>\n</head>\n\n"
           "<body bgcolor=\"#ffffff\" >\n"
           "<H2>Insdex of %s</H2>\n"
           "<table>\n%s",
           title, title,
	...
	/*加了个%s <- dir 显示相对路径*/
        printf("<tr>"
               "<td width=\"40%%\"><a href=\"%s%s/\">%s/</a></td>"
               "<td align=right>%s</td>"
               "<td align=right>%ld bytes</td>"
               "</tr>\n",
               dir,http_filename, html_filename,
...
/*加了个 %s <- dir 显示 http_filename*/
            printf("<tr>"
                   "<td width=\"40%%\"><a href=\"%s%s\">%s</a> "
                   "<a href=\"%s.gz\">(.gz)</a></td>"
                   "<td align=right>%s</td>"
                   "<td align=right>%ld bytes</td>"
                   "</tr>\n",
                   dir,http_filename, html_filename, http_filename,
                   ctime(&statbuf.st_mtime), (long) statbuf.st_size);
        } else {
#endif
/*加了个 %s <- dir 显示 http_filename*/
